...
# 3. p, indicating that hthe new content of the original line should be printed
# 4. w file, which means to write hthe result of the substitution to a file

echo 
echo "Example-1: sed substition, s, command default"
echo 

# the sed substitution flag, s, only replaces the first occurence of the 'test'on each line
# when there are more than one as shown in this example
...
# Only the first occurences of the word 'test' on each line was replaced and writes it to a file
...
# Replaces all occurences of the word 'test' and writes it to a file
# creates the file if it does not exist
